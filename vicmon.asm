...
;
; Converted to nasm syntax by FozzTexx 2022-Jan-23
; 
; Victor 9000 serial I/O added by FozzTexx
...
TARBELLDD:	EQU	0	;Tarbell DD controller SD or DD?
...
;; INITPORT:
;; 	CS	LODSB		;Get byte count
;; 	MOV	CL,AL
;; 	JCXZ	NEXTPORT	;No init. for some ports
;; INITBYTE:
;; 	CS	LODSB		;Get init. data
...
;Re-establish initial conditions
	...
	MOV	ES,AX
	MOV	SP,STACK
	;; MOV	word [64H],INT0	;Set UART interrupt vector
	;; MOV	[66H],CS
	MOV	AL,PROMPT
	CALL	OUT
...
;If we got here, character was invalid. Sound bell.
	MOV	AL,7
	CALL	OUT
	JCXZ	WAIT0		;CX=0 means no more digits
...
HEADER:	DB	13,10,10,"SCP 8086 Monitor 1.5",13,10+128
SYNERR:	DB	'^'
ERRMES:	DB	" Error",13,10+128
BACMES:	DB	8,32,8+128
...
DISK:	EQU	0E0H
	...
	MOV	DH,AL		; Put drive select byte in DH.
	%ENDIF

	%IF	CROMEMCO4FDC+CROMEMCO16FDC
DISK:	EQU	30H
	MOV	AL,1
	OUT	2,AL		;Reset 4FDC serial I/O
	MOV	AL,84H
	OUT	0,AL		;and set for 300 baud
	MOV	AL,7FH
	OUT	4,AL
	MOV	DH,21H		; Drive-select byte.
	%ENDIF
...
RETRY:
	...
	%IF	SCP
	XOR	DH,08H		; If SCP, flip density bit.
	%ENDIF

	%IF	CROMEMCO16FDC+CROMEMCO4FDC
	XOR	DH,10H		; If Cromemco, flip disk size bit.
	%ENDIF
...
BRKCNT:	RESW	2		;Number of breakpoints
TCOUNT:	RESW	2		;Number of steps to trace
BPTAB:	RESW	BPLEN		;Breakpoint table
LINEBUF:RESW	BUFLEN+1	;Line input buffer
